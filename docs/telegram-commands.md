# Telegram Commands

| Command | Purpose |
|---|---|
| `/start` | Show available Quantara commands. |
| `/scan SOL` | Run full signal pipeline. |
| `/scalp SOLUSDT` | Run scalp-biased signal pipeline. |
| `/risk 1000 2` | Calculate max loss for capital and risk percent. |
| `/watch BTC ETH SOL` | Add symbols to watchlist. |
| `/unwatch SOL` | Disable a symbol from watchlist. |
| `/watchlist` | Show active watchlist entries. |
| `/settings` | Show current settings. |
| `/settings risk 1.5` | Update risk per trade. |
